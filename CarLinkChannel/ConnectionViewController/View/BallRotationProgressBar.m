...
@interface BallRotationProgressBar ()
...
@implementation BallRotationProgressBar
...
- (CAShapeLayer *)oneLayer{
    if (!_oneLayer) {
        ...
        _oneLayer.fillColor = [UIColor whiteColor].CGColor;
...
- (CAShapeLayer *)middleLayer{
    if(!_middleLayer){
        ...
        _middleLayer.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
        _middleLayer.fillColor = [UIColor whiteColor].CGColor;
        _middleLayer.path = [UIBezierPath bezierPathWithArcCenter:self.centerPoint radius:self.radius startAngle:0 endAngle:M_PI*2 clockwise:YES].CGPath;
    }
    return _middleLayer;
}
- (CAShapeLayer *)twoLayer{
    if (!_twoLayer) {
       ...
       // _twoLayer.anchorPoint = CGPointMake(0.5, 0.5);
        _twoLayer.fillColor = [UIColor whiteColor
        ].CGColor;
...
- (void)initProgressBar{    
    ...
    self.radius = DEFAULT_RADIUS / 4 * 3;
...
- (void)setBallRadius:(CGFloat)radius{
    ...
    self.middleLayer.path = [UIBezierPath bezierPathWithArcCenter:self.centerPoint radius:self.radius startAngle:0 endAngle:M_PI*2 clockwise:YES].CGPath;
...
- (void)setOneBallColor:(UIColor *)oneColor twoBallColor:(UIColor *)twoColor{
    ...
    self.middleLayer.fillColor = [UIColor redColor].CGColor;
}
...
-(void)startMiddleBallAnimator{
    ...
    CAKeyframeAnimation *oneFrameAnimation = [CAKeyframeAnimation animationWithKeyPath:@"position"];
    CGMutablePathRef path = CGPathCreateMutable();
    CGPathMoveToPoint(path, NULL, self.centerPoint.x+self.distance, self.centerPoint.y);
    CGPathAddLineToPoint(path, NULL, self.centerPoint.x, self.centerPoint.y);
    CGPathAddLineToPoint(path, NULL, self.centerPoint.x-self.distance, self.centerPoint.y);
    CGPathAddLineToPoint(path, NULL, self.centerPoint.x, self.centerPoint.y);
    CGPathAddLineToPoint(path, NULL, self.centerPoint.x+self.distance, self.centerPoint.y);
    ...
    //第一个小球缩放动画
    CAKeyframeAnimation *oneScaleAnimation = [CAKeyframeAnimation animationWithKeyPath:@"transform.scale"];
    oneScaleAnimation.values = @[@1,@0.5,@1,@1,@1];
...
//    第一个小球透明动画
    ...
    //组合动画
    CAAnimationGroup *oneGroup = [CAAnimationGroup animation];
    [oneGroup setAnimations:@[transformAnimation,oneFrameAnimation]];
    oneGroup.duration = self.duration;
    oneGroup.repeatCount = HUGE;
    [self.middleLayer addAnimation:oneGroup forKey:@"oneGroup"];
