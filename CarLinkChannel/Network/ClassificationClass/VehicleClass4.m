...
//                [operationsSecurity addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x02 :5000 :YES];}];              //Enter Programming session
                [operationsSecurity addObject:^OperationResult *{return [self.ProtocolDispatcher applySecurityProtocol:Vehicle1SecurityType11 :VehicleVersionInfo :30000];}];                     //security process
                [operationsSecurity addObject:^OperationResult *{return [self.ProtocolDispatcher WriteIdentifier:0xF15a :[NSData dataWithBytes:(uint8_t[]){0x17,0x11 ,0x30,0x8f,0x04,0xd2,0x01,0x00,0x00,0x10,0x00,0x00,0x00} length:13] :30000 :YES];}];
            ...
            if(BlockInfo.RoutineControlState == 1){
                ...
                usleep(10000);
                result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :routinDataCopy :30000 :YES CheckType:0];} delegate:self.delegate];
...
//                [operations3103 addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0xFF,0x00,0x02,0x40,0x09,0x77,0xfd,0x00} length:8] :3000 :YES];}];
//                [operations3103 addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0xFF,0x00,0x02,0x40,0x09,0x67,0xfd,0x00} length:8] :3000 :YES];}];
//                [operations3103 addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0xFF,0x00,0x02,0x40,0x09,0x03,0xfd,0x00} length:8] :3000 :YES];}];
...
//                 }
            ...
            // Send ready for play
            result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher RequestStartDownloadToFlash:BlockInfo.RequestDownloadFormatID :0x44 :BlockInfo.BlockStartAddr :BlockInfo.BlockLength :30000 :YES];} delegate:self.delegate];       //34
            if(!result.state)
                return;
            ...
            [operationsFlashData addObject:^OperationResult *{return [self.ProtocolDispatcher sendExitTrans:30000 :YES];}];    //exit trans
            ...
            if(BlockInfo.CheckOperationState){
                ...
                result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :SendDataCopy :30000 :YES CheckType:0];} delegate:self.delegate];
                if(!result.state)
                    return;
                
                if(BlockInfo.FF01State !=0x02)
                {
                    result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0xff,0x01} length:2] :30000 :YES CheckType:0];} delegate:self.delegate];
                    if(!result.state)
                        return;
                }
            }
            ...
            if(BlockInfo.IsDefaultSessionRequired){
                result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher ResetEcu:30000 :NO :15];} delegate:self.delegate];
                if(!result.state)
                    return;
            }
            if(BlockInfo.SendOtherState){
                NSMutableArray *operationsOther = [NSMutableArray array];
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x03 :30000 :NO];}];         //Enter Extended Diagnostic Session 10 03
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0x0F,0x0c,0x03} length:3] :30000 :NO CheckType:0];}];
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher SetDTCState:false :30000 :NO];}];
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher SetNormalCommunicationState:[NSData dataWithBytes:(uint8_t[]){0x01} length:1] :30000 :NO];}];
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0x10,0x03,0x01} length:3] :30000 :NO CheckType:0];}];
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x01 :30000 :NO];}];         //Enter default session
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x03 :30000 :NO];}];         //Enter Extended Diagnostic Session
        ...
        if(Datamanager.EndState1){
            ...
            [operationsEnd addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0xFF,0x01} length:2] :30000  :NO CheckType:0];}];
            ...
            [operationsEnd addObject:^OperationResult *{return [self.ProtocolDispatcher ResetEcu:10000 :NO :15];}];
            ...
            [operationsEnd addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x01 :10000 :NO];}];         //Enter default session
            [operationsEnd addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x03 :10000 :NO];}];         //Enter Extended Diagnostic Session
        ...
        if(Datamanager.EndState2){
            ...
            [operationsEnd2 addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x41 :10000 :NO];}];
            [operationsEnd2 addObject:^OperationResult *{return [self.ProtocolDispatcher applySecurityProtocol:Vehicle1SecurityType01 :VehicleVersionInfo :10000];}];                     //security process
        ...
        if(Datamanager.EndState4){
            ...
            [operationsEnd4 addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0x0f,0x01} length:2] :10000 :NO CheckType:0];}];
            NSData *SendVindata = [VehcileVin dataUsingEncoding:NSASCIIStringEncoding];
            SendVindata = [SendVindata subdataWithRange:NSMakeRange(SendVindata.length - 7, 7)];
            [operationsEnd4 addObject:^OperationResult *{return [self.ProtocolDispatcher WriteIdentifier:0x37FE :SendVindata :10000 :NO];}];
            ...
            [operationsEnd4 addObject:^OperationResult *{return [self.ProtocolDispatcher ReadEcuInfo:[NSData dataWithBytes:(uint8_t[]){0x01} length:1] :10000 :NO];}];
...
-(NSUInteger)GetAllStepCount:(NSArray *)array{
    ...
    for(FlashBlockInfo *codeInfo in array){
        ...
        long numberOfPackets = GetBlockLgenth/1792;
        long lastPacketSize = GetBlockLgenth%1792;
...
-(NSArray *)GetFlashData:(NSData *)SendData{
    ...
    long numberOfPackets =SendData.length/1792;
    long lastPacketSize = SendData.length%1792;
    for(int i = 0;i<numberOfPackets;i++)
    {
        FlashInfo *infor = [[FlashInfo alloc] init];
        infor.index = i + 1;
        infor.data = [SendData subdataWithRange:NSMakeRange(0 + i*1792, 1792)];
    ...
    if(lastPacketSize>0)
    {
        ...
        infor.data = [SendData subdataWithRange:NSMakeRange(numberOfPackets*1792, lastPacketSize)];
        [CodeArray addObject:infor];
    }
    return CodeArray;
}
