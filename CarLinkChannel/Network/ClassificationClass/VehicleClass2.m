...
-(void)LoadFileToVehicle:(NSString *)FilePath :(NSString *)VehcileVin :(NSArray *)VehicleVersionInfo :(NSArray *)CafdInfo{
    ...
    if(Datamanager.isValidBinFile){
        if(Datamanager.SetState2){
            ...
            [operationsSteg1 addObject:^OperationResult*{return [self.ProtocolDispatcher changeDiagnosticSession:0x01 :10000 :NO];}]; //Enter default session 10 01
            ...
            [operationsSteg1 addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x03 :10000 :NO];}];//Enter extended diagnostic 10 03 session
...
//                [operationsSecurity addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x02 :5000 :YES];}]; //Enter Programming session 10 02
                ...
                [operationsSecurity addObject:^OperationResult *{return [self.ProtocolDispatcher WriteIdentifier:0xF15a :[NSData dataWithBytes:(uint8_t[]){0x19,0x03,0x05,0x8d,0x02,0x62,0x01,0x00,0x00,0x00,0x35,0x00,0x00} length:13] :5000 :NO];}];
            ...
            if(BlockInfo.RoutineControlState == 1){
                ...
                usleep(10000);
                result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :routinDataCopy :5000 :YES CheckType:0];} delegate:self.delegate];
        ...
        if(Datamanager.EndState4){
            ...
            [operationsEnd4 addObject:^OperationResult *{return [self.ProtocolDispatcher ResetEcu:3000 :NO :7];}];
