...
-(NSDictionary *)ReadHealth{
    ...
    {
        NSUInteger length = [PressureResult.receiveData length];
        NSAssert(length % 2 == 0, @"数据长度必须是偶数");

        NSMutableData *convertedData = [NSMutableData dataWithCapacity:length];
        ...
        for (NSUInteger i = 0; i < length; i += 2) {
            ...
            // 从 NSData 中提取两个字节的数据
            [PressureResult.receiveData getBytes:&value range:range];
            
            // 将提取的 uint16_t 数据转换为不同的字节序
            value = CFSwapInt16LittleToHost(value);

            // 拆分为两个 uint8_t
            uint8_t highByte = (value >> 8) & 0xFF; // 高字节
            uint8_t lowByte = value & 0xFF;         // 低字节
            
            // 将转换后的字节添加到 NSMutableData
            [convertedData appendBytes:&highByte length:1];
            [convertedData appendBytes:&lowByte length:1];
        }

        NSLog(@"Data: %@", convertedData);
        ResultDic[@"pressure"] = convertedData;
    }
    OperationResult *TimeResult = [self.ProtocolDispatcher ReadTcuHealth:1];
    if(TimeResult.state == YES)
    {
        NSUInteger length = [TimeResult.receiveData length];
        NSAssert(length % 2 == 0, @"数据长度必须是偶数");
        ...
        for (NSUInteger i = 0; i < length; i += 2) {
            ...
            // 从 NSData 中提取两个字节的数据
            [TimeResult.receiveData getBytes:&value range:range];
...
-(void)LoadFileToVehicle:(NSString *)FilePath :(NSString *)VehcileVin :(NSArray *)VehicleVersionInfo :(NSArray *)CafdInfo{
    NSLog(@"Array: %@", VehicleVersionInfo);
    
    NSString *BtldInfo = VehicleVersionInfo[1];
    NSLog(@"BTLD: %@", BtldInfo);
    ...
    if(Datamanager.isValidBinFile){
        if(Datamanager.SetState2)
        {
            ...
            [operationsSteg1 addObject:^OperationResult*{return [self.ProtocolDispatcher changeDiagnosticSession:0x01 :5000 :NO];}];                  //Enter default session
            [operationsSteg1 addObject:^OperationResult*{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0x02, 0x03} length:2] :3000 :NO CheckType:0];}];       //routine control 0x01 0x02, 0x03
            [operationsSteg1 addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x03 :5000 :NO];}];                  //Enter extended diagnostic session
            [operationsSteg1 addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0x0f, 0x0c,0x03} length:3] :3000 :NO CheckType:0];}];  //routine control 0x01 0x0f, 0x0c,0x03
            [operationsSteg1 addObject:^OperationResult *{return [self.ProtocolDispatcher SetDTCState:false :5000 :NO];}];                             //close DTC (Diagnostic Toruble Code) 85 02
            [operationsSteg1 addObject:^OperationResult *{return [self.ProtocolDispatcher SetNormalCommunicationState:[NSData dataWithBytes:(uint8_t[]){0x01} length:1] :5000 :NO];}];//close CAN 28 01 01
            [operationsSteg1 addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0x10,0x03,0x01} length:3]  :3000 :NO CheckType:0];}];  //routine control 0x01 0x10,0x03,0x01
        ...
        for(FlashBlockInfo *BlockInfo in Datamanager.BlockInfo)
        {
            if(BlockInfo.SecurityState){                                                                //if need 27(security)
                ...
                [operationsSecurity addObject:^OperationResult *{return [self.ProtocolDispatcher applySecurityProtocol:Vehicle1SecurityType11 :VehicleVersionInfo :5000];}];                     //security process
                [operationsSecurity addObject:^OperationResult *{return [self.ProtocolDispatcher WriteIdentifier:0xF15a :[NSData dataWithBytes:(uint8_t[]){0x19,0x03 ,0x05,0x8d,0x02,0x62,0x01,0x00,0x00,0x00,0x35,0x00,0x00} length:13] :5000 :YES];}];
            ...
            if(BlockInfo.CheckOperationState){
                ...
                result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :SendDataCopy :10000 :YES CheckType:1];} delegate:self.delegate];
                if(!result.state)
                    return;
                
                if(BlockInfo.FF01State !=0x02)
                {
                    result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0xff,0x01} length:2] :10000 :NO CheckType:0];} delegate:self.delegate];
                    if(!result.state)
                        return;
                }
            ...
            if(BlockInfo.IsDefaultSessionRequired){
                result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher ResetEcu:10000 :NO :7];} delegate:self.delegate];
                if(!result.state)
                    return;
            }
        ...
        result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher SendCafdToEcu:SendCafd :10000 :YES];} delegate:self.delegate];
        if(!result.state)
            return;
        ...
        if(Datamanager.EndState4){
            ...
            [operationsEnd4 addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x01 :10000 :NO];}];
            [operationsEnd4 addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x02 :10000 :NO];}];
            [operationsEnd4 addObject:^OperationResult *{return [self.ProtocolDispatcher ResetEcu:10000 :NO :5];}];         //Enter default session
...
-(void)RecoveryCafdToVehicle:(NSString *)VehcileVin :(NSArray *)VehicleVersionInfo :(NSArray *)CafdInfo{
    ...
    NSMutableArray *operationsOther = [NSMutableArray array];
    [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher ResetEcu:3000 :YES :7];}];                             //Reset
