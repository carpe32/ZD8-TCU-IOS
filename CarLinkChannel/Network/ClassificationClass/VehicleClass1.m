...
-(NSArray *)ReadCafd{
    return [self.ProtocolDispatcher ReadCafd:4 :3000];
}
...
//                [operationsSecurity addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x02 :5000 :YES];}];              //Enter Programming session
                ...
                [operationsSecurity addObject:^OperationResult *{return [self.ProtocolDispatcher WriteIdentifier:0xF15a :[NSData dataWithBytes:(uint8_t[]){0x17,0x11,0x30,0x8f,0x04,0xd2,0x01,0x00,0x00,0x0B,0xBB,0x00,0x00} length:13] :10000 :YES];}];
            ...
            // Send ready for play
            result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher RequestStartDownloadToFlash:BlockInfo.RequestDownloadFormatID :0x44 :BlockInfo.BlockStartAddr :BlockInfo.BlockLength :5000 :YES];} delegate:self.delegate];       //34
            if(!result.state)
                return;
            ...
            if(BlockInfo.CheckOperationState){
                ...
                {
                    result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0xff,0x01} length:2] :3000 :YES CheckType:0];} delegate:self.delegate];
                    if(!result.state)
                        return;
                }
            }
            ...
            if(BlockInfo.IsDefaultSessionRequired){
                result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher ResetEcu:3000 :NO :5];} delegate:self.delegate];
                if(!result.state)
                    return;
            }
            if(BlockInfo.SendOtherState){
                NSMutableArray *operationsOther = [NSMutableArray array];
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x03 :5000 :NO];}];         //Enter Extended Diagnostic Session 10 03
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0x0F,0x0c,0x03} length:3] :3000 :NO CheckType:0];}];
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher SetDTCState:false :5000 :NO];}];
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher SetNormalCommunicationState:[NSData dataWithBytes:(uint8_t[]){0x01} length:1] :5000 :NO];}];
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0x10,0x03,0x01} length:3] :3000 :NO CheckType:0];}];
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x01 :5000 :NO];}];         //Enter default session
                [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x03 :5000 :NO];}];         //Enter Extended Diagnostic Session
        ...
        if(Datamanager.EndState1){
            ...
            [operationsEnd addObject:^OperationResult *{return [self.ProtocolDispatcher RoutineControlRequest:0x01 :[NSData dataWithBytes:(uint8_t[]){0xFF,0x01} length:2] :3000  :NO CheckType:0];}];
            
            NSData *SendVindata = [VehcileVin dataUsingEncoding:NSASCIIStringEncoding];
            [operationsEnd addObject:^OperationResult *{return [self.ProtocolDispatcher WriteIdentifier:0xF190 :SendVindata :5000 :NO];}];
            [operationsEnd addObject:^OperationResult *{return [self.ProtocolDispatcher ResetEcu:3000 :NO :7];}];
            ...
            [operationsEnd addObject:^OperationResult *{return [self.ProtocolDispatcher ReadEcuInfo:[NSData dataWithBytes:(uint8_t[]){0x01} length:1] :5000 :NO];}];
        ...
        result = [self.OperationResult executeOperationWithBlock:^OperationResult *{return [self.ProtocolDispatcher SendCafdToEcu:SendCafd :5000 :YES];} delegate:self.delegate];
        if(!result.state)
            return;
...
-(void)RecoveryCafdToVehicle:(NSString *)VehcileVin :(NSArray *)VehicleVersionInfo :(NSArray *)CafdInfo{
    ...
    NSData *SendVindata = [VehcileVin dataUsingEncoding:NSASCIIStringEncoding];
    SendVindata = [SendVindata subdataWithRange:NSMakeRange(SendVindata.length - 7, 7)];
    [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher WriteIdentifier:0x37FE :SendVindata :5000 :NO];}];
    [operationsOther addObject:^OperationResult *{return [self.ProtocolDispatcher changeDiagnosticSession:0x01 :5000 :NO];}];         //Enter default session
