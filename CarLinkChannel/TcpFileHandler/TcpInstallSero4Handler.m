//
//  TcpInstallSero4handler.m
//  CarLinkChannel
//
//  Created by job on 2023/4/21.
//

#import "TcpInstallSero4Handler.h"
#import "HttpClient.h"
...
@interface TcpInstallSero4Handler()
{
    __block OSSpinLock oslock;
    NSString * receiveanquan;
}
@end
@implementation TcpInstallSero4Handler
...
-(BOOL)sendCommonData:(NSString *)datapacket {
   ...
   // NSLog(@"发送: %@",datapacket);
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(Sero4sendInstallControllerPacket:)]){
        NSLog(@"--------------------> 发送数据");
//        sleep(1);
        usleep(200000);
        return  [self.delegate Sero4sendInstallControllerPacket:datapacket];
    }
//
//    NSString * unitString = @"";
//
       ...
       // }
        ...
        if([front2 isEqualToString:condition_on]){
           ...
           BOOL result = [self sendCommonData:front2_data_packet_1];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0401" forKey:@"ErrorCode"];
            ...
            if(result == FALSE){
                                [ErroruserInfo setObject:@"0402" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_3];
            if(result == FALSE){
                                [ErroruserInfo setObject:@"0403" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_4];
            if(result == FALSE){
                                [ErroruserInfo setObject:@"0404" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_5];
            if(result == FALSE){
                                [ErroruserInfo setObject:@"0405" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_6];
            if(result == FALSE){
                                [ErroruserInfo setObject:@"0406" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =  [self sendCommonData:front2_data_packet_7];
            if(result == FALSE){
                                [ErroruserInfo setObject:@"0407" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
        ...
        for (int i = 0; i < 14; i++) {
            ...
            if([iswrite isEqualToString:attribute_01]){
                if([issafe isEqualToString:condition_on]){
                    ...
                    if(result == false){
                                        [ErroruserInfo setObject:@"0408" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
...
//
                    OSSpinLockLock(&oslock);
                    if(result == false){
                                        [ErroruserInfo setObject:@"0409" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  ...
                  result =  [self sendCommonData:sero_4_datapacket_1];
                    if(result == false){
                                        [ErroruserInfo setObject:@"0410" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  //  [];
                ...
                if([issend isEqualToString:attribute_01]){
                   ...
                   BOOL result =  [self sendCommonData:dataString];
                    if(result == false){
                                        [ErroruserInfo setObject:@"0411" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }else if ([issend isEqualToString:attribute_03]){
//                    [self sendCommonData:send_data_packet_3_1];
//                    [self sendCommonData:send_data_packet_3_2];
//                    [self sendCommonData:send_data_packet_3_3];
                ...
                if(result == false){
                                    [ErroruserInfo setObject:@"0412" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
            ...
            //    NSLog(@"binString: %@",binString);
                ...
                // 开始发送文件
                long times = binData.length / 1792;
                long last = binData.length % 1792;
                ...
                if(times != 0){
                    for (int i = 0; i < times; i++) {
                        ...
                        NSString * data_packet = [sero_4_bin_prefix stringByAppendingString:pos_hex];
                        tempdata = [binData subdataWithRange:NSMakeRange(i*1792, 1792)];
                        ...
                        if(!result){
                                            [ErroruserInfo setObject:@"0413" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                            return;
                        }else{
...
//                        sleep(0.5);
                ...
                if(last > 0){
                    ...
                    tempdata = [binData subdataWithRange:NSMakeRange(times * 1792, last)];
                    ...
                    bool result =  [self sendCommonBinaryData:data_packet];
                    if(!result){
                                        [ErroruserInfo setObject:@"0414" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }else{
...
//                    sleep(0.5);
              ...
              result =  [self sendCommonData:send_data_end_1];
                if(result == false){
                                    [ErroruserInfo setObject:@"0415" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
              result =  [self sendCommonData:send_data_end_2];
                if(result == false){
                                    [ErroruserInfo setObject:@"0416" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
                ...
                if([ischecksuccess isEqualToString:attribute_01]){
                    ...
                    if(resu){
                        if([succsendend isEqualToString:attribute_00]){
                            ...
                            if(result == false){
                                                [ErroruserInfo setObject:@"0417" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                                return;
                            }
                        }
                ...
                if([is1101 isEqualToString:attribute_01]){
                  ...
                  result =  [self sendCommonData:send_1101_data_packet];
                    if(result == false){
                                        [ErroruserInfo setObject:@"0418" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }
                ...
                if([isspecdata isEqualToString:attribute_01]){
                   ...
                   result = [self sendCommonData:send_spec_data_packet_1];
                    if(result == false){
                                        [ErroruserInfo setObject:@"0419" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                   result = [self sendCommonData:send_spec_data_packet_2];
                    if(result == false){
                                        [ErroruserInfo setObject:@"0420" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                   result = [self sendCommonData:send_spec_data_packet_3];
                    if(result == false){
                                        [ErroruserInfo setObject:@"0421" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                   result = [self sendCommonData:send_spec_data_packet_4];
                    if(result == false){
                                        [ErroruserInfo setObject:@"0422" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                   result = [self sendCommonData:send_spec_data_packet_5];
                    if(result == false){
                                        [ErroruserInfo setObject:@"0423" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                   result = [self sendCommonData:send_spec_data_packet_6];
                    if(result == false){
                                        [ErroruserInfo setObject:@"0424" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                   result = [self sendCommonData:send_spec_data_packet_7];
                    if(result == false){
                                        [ErroruserInfo setObject:@"0425" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }
        ...
        if([end1 isEqualToString:attribute_01]){
           ...
           BOOL result = [self sendCommonData:send_end_1_packet_1];
            if(result == false){
                                [ErroruserInfo setObject:@"0426" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
          ...
          result =  [self sendCommonData:datapcket];
            if(result == false){
                                [ErroruserInfo setObject:@"0427" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
          result =  [self sendCommonData:send_end_1_packet_3];
            if(result == false){
                                [ErroruserInfo setObject:@"0428" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
          result =  [self sendCommonData:send_end_1_packet_4];
            if(result == false){
                                [ErroruserInfo setObject:@"0429" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
          result =  [self sendCommonData:send_end_1_packet_5];
            if(result == false){
                                [ErroruserInfo setObject:@"0430" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
          result =  [self sendCommonData:send_end_1_packet_6];
            if(result == false){
                                [ErroruserInfo setObject:@"0431" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
          result =  [self sendCommonData:send_end_1_packet_7];
            if(result == false){
                                [ErroruserInfo setObject:@"0432" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
          result =  [self sendCommonData:send_end_1_packet_8];
            if(result == false){
                                [ErroruserInfo setObject:@"0433" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
        ...
        if([end2 isEqualToString:attribute_01]){
            ...
            if(result == false){
                                [ErroruserInfo setObject:@"0434" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            ...
            OSSpinLockLock(&oslock);
            if(result == false){
                                [ErroruserInfo setObject:@"0435" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
...
//                [self sendCommonData:dataString];
//            }
//            
//        }
        ...
        if([end4 isEqualToString:attribute_01]){
           ...
           BOOL result = [self sendCommonData:send_end_4_packet_1];
            if(result == false){
                                [ErroruserInfo setObject:@"0436" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
...
//            [self sendCommonData:send_end_4_packet_2_unit_1];
           result =  [self sendCommonData:dataPacket];
            if(result == false){
                                [ErroruserInfo setObject:@"0437" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_4_packet_3];
            if(result == false){
                                [ErroruserInfo setObject:@"0438" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_4_packet_4];
            if(result == false){
                                [ErroruserInfo setObject:@"0439" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_4_packet_5];
            if(result == false){
                                [ErroruserInfo setObject:@"0440" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_4_packet_6];
            if(result == false){
                                [ErroruserInfo setObject:@"0441" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
