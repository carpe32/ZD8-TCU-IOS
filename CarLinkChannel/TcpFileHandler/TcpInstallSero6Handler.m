...
#import "TcpInstallSero6Handler.h"
#import "HttpClient.h"
...
@interface TcpInstallSero6Handler()
{
    __block OSSpinLock oslock;
    NSString * receiveanquan;
}
@end
@implementation TcpInstallSero6Handler
      ...
      //  if([front1 isEqualToString:condition_on]){
       ...
       // }
        ...
        BOOL result =  [self sendCommonData:front2_data_packet_1];
            if(result == false){
                [ErroruserInfo setObject:@"0601" forKey:@"ErrorCode"];
            ...
            if(result == false){
                [ErroruserInfo setObject:@"0602" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =   [self sendCommonData:front2_data_packet_3];
            if(result == false){
                [ErroruserInfo setObject:@"0603" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =   [self sendCommonData:front2_data_packet_4];
            if(result == false){
                [ErroruserInfo setObject:@"0604" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =   [self sendCommonData:front2_data_packet_5];
            if(result == false){
                [ErroruserInfo setObject:@"0605" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =   [self sendCommonData:front2_data_packet_6];
            if(result == false){
                [ErroruserInfo setObject:@"0606" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =   [self sendCommonData:front2_data_packet_7];
            if(result == false){
                [ErroruserInfo setObject:@"0607" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
        ...
        for (int i = 0; i < 14; i++) {
            ...
            if([iswrite isEqualToString:attribute_01]){
                if([issafe isEqualToString:condition_on]){
                    ...
                    if(result == false){
                        [ErroruserInfo setObject:@"0608" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
...
//
                    OSSpinLockLock(&oslock);
                    if(result == false){
                        [ErroruserInfo setObject:@"0609" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  ...
                  result =  [self sendCommonData:sero_6_datapacket_1];
                    if(result == false){
                        [ErroruserInfo setObject:@"0610" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  //  [];
                ...
                if([issend isEqualToString:attribute_01]){
                   ...
                   BOOL result =  [self sendCommonData:dataString];
                    if(result == false){
                        [ErroruserInfo setObject:@"0611" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                   ...
                   BOOL result = [self sendCommonData:send_data_packet_3_1];
                    if(result == false){
                        [ErroruserInfo setObject:@"0612" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  result =  [self sendCommonData:send_data_packet_3_2];
                    if(result == false){
                        [ErroruserInfo setObject:@"0613" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  result =  [self sendCommonData:send_data_packet_3_3];
                    if(result == false){
                        [ErroruserInfo setObject:@"0614" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                ...
                if(result == false){
                    [ErroruserInfo setObject:@"0615" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
            ...
            //    NSLog(@"binString: %@",binString);
                ...
                if(times != 0){
                    for (int i = 0; i < times; i++) {
                        ...
                        if(!result){
                            [ErroruserInfo setObject:@"0616" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                            return;
                        }else{
...
//                        sleep(0.5);
                ...
                if(last > 0){
                    ...
                    bool result =  [self sendCommonBinaryData:data_packet];
                    if(!result){
                        [ErroruserInfo setObject:@"0617" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }else{
...
//                    sleep(0.5);
               ...
               result = [self sendCommonData:send_data_end_1];
                if(result == false){
                    [ErroruserInfo setObject:@"0618" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
               result = [self sendCommonData:send_data_end_2];
                if(result == false){
                    [ErroruserInfo setObject:@"0619" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
                    ...
                    if(resu){
                        if([succsendend isEqualToString:attribute_00]){
                           ...
                           result = [self sendCommonData:check_succ_end_packet_2];
                            if(result == false){
                                [ErroruserInfo setObject:@"0620" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                                return;
                            }
                        }
                ...
                if([is1101 isEqualToString:attribute_01]){
                   ...
                   result = [self sendCommonData:send_1101_data_packet];
                    if(result == false){
                        [ErroruserInfo setObject:@"0621" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }
                ...
                if([isspecdata isEqualToString:attribute_01]){
                  ...
                  result =   [self sendCommonData:send_spec_data_packet_1];
                    if(result == false){
                        [ErroruserInfo setObject:@"0622" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  result =  [self sendCommonData:send_spec_data_packet_2];
                    if(result == false){
                        [ErroruserInfo setObject:@"0623" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                  ...
                  result =  [self sendCommonData:send_spec_data_packet_4];
                    if(result == false){
                        [ErroruserInfo setObject:@"0625" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  result =  [self sendCommonData:send_spec_data_packet_5];
                    if(result == false){
                        [ErroruserInfo setObject:@"0626" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  result =  [self sendCommonData:send_spec_data_packet_6];
                    if(result == false){
                        [ErroruserInfo setObject:@"0627" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  result =  [self sendCommonData:send_spec_data_packet_7];
                    if(result == false){
                        [ErroruserInfo setObject:@"0628" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }
        ...
        if([end1 isEqualToString:attribute_01]){
           ...
           BOOL result = [self sendCommonData:send_end_1_packet_1];
            if(result == false){
                [ErroruserInfo setObject:@"0629" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            ...
            if(result == false){
                [ErroruserInfo setObject:@"0630" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_1_packet_3];
            if(result == false){
                [ErroruserInfo setObject:@"0631" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_1_packet_4];
            if(result == false){
                [ErroruserInfo setObject:@"0632" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_1_packet_5];
            if(result == false){
                [ErroruserInfo setObject:@"0633" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_1_packet_6];
            if(result == false){
                [ErroruserInfo setObject:@"0634" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_1_packet_7];
            if(result == false){
                [ErroruserInfo setObject:@"0635" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_1_packet_8];
            if(result == false){
                [ErroruserInfo setObject:@"0636" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
        ...
        if([end2 isEqualToString:attribute_01]){
            ...
            if(result == false){
                [ErroruserInfo setObject:@"0637" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            ...
            OSSpinLockLock(&oslock);
            if(result == false){
                [ErroruserInfo setObject:@"0638" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
...
//        NSString * path = [[NSUserDefaults standardUserDefaults] objectForKey:vin_file_path];
        ...
        for (NSString * subString in cafdString) {
            if(subString.length > 0){
                ...
                BOOL result = [self sendCommonData:dataString];
                if(result == false){
                    [ErroruserInfo setObject:@"0639" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
            }
        ...
        if([end4 isEqualToString:attribute_01]){
          ...
          BOOL result =  [self sendCommonData:send_end_4_packet_1];
            if(result == false){
                [ErroruserInfo setObject:@"0640" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
...
//            [self sendCommonData:send_end_4_packet_2_unit_1];
            ...
            if(result == false){
                [ErroruserInfo setObject:@"0641" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_4_packet_3];
            if(result == false){
                [ErroruserInfo setObject:@"0642" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_4_packet_4];
            if(result == false){
                [ErroruserInfo setObject:@"0643" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_4_packet_5];
            if(result == false){
                [ErroruserInfo setObject:@"0644" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_4_packet_6];
            if(result == false){
                [ErroruserInfo setObject:@"0645" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
