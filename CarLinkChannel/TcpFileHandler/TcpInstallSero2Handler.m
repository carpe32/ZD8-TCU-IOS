//
//  TcpInstallSero2Handler.m
//  CarLinkChannel
//
//  Created by job on 2023/4/21.
//

#import "TcpInstallSero2Handler.h"
#import "HttpClient.h"
...
@interface TcpInstallSero2Handler()
{
    __block OSSpinLock oslock;
    NSString * receiveanquan;
}
@end
@implementation TcpInstallSero2Handler
       ...
       // }
        ...
        if([front2 isEqualToString:condition_on]){
          ...
          BOOL result = [self sendCommonData:front2_data_packet_1];
            if(result == false){
                [ErroruserInfo setObject:@"0201" forKey:@"ErrorCode"];
            ...
            if(result == false){
                [ErroruserInfo setObject:@"0202" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_3];
            if(result == false){
                [ErroruserInfo setObject:@"0203" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_4];
            if(result == false){
                [ErroruserInfo setObject:@"0204" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_5];
            if(result == false){
                [ErroruserInfo setObject:@"0205" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_6];
            if(result == false){
                [ErroruserInfo setObject:@"0206" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_7];
            if(result == false){
                [ErroruserInfo setObject:@"0207" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
        ...
        for (int i = 0; i < 14; i++) {
            ...
            if([iswrite isEqualToString:attribute_01]){
                if([issafe isEqualToString:condition_on]){
                    ...
                    if(result == false){
                        [ErroruserInfo setObject:@"0208" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
...
//
                    OSSpinLockLock(&oslock);
                    if(result == false){
                        [ErroruserInfo setObject:@"0209" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  ...
                  result =  [self sendCommonData:sero_2_datapcket_1];
                    if(result == false){
                        [ErroruserInfo setObject:@"0210" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  //  [];
                ...
                if([issend isEqualToString:attribute_01]){
                   ...
                   BOOL result = [self sendCommonData:dataString];
                    if(result == false){
                        [ErroruserInfo setObject:@"0211" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    ...
                    if(result == false){
                        [ErroruserInfo setObject:@"0212" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_data_packet_3_2];
                    if(result == false){
                        [ErroruserInfo setObject:@"0213" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_data_packet_3_3];
                    if(result == false){
                        [ErroruserInfo setObject:@"0214" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }else{
                ...
                if(result == false){
                    [ErroruserInfo setObject:@"0215" forKey:@"ErrorCode"];
                    [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
            ...
            //    NSLog(@"binString: %@",binString);
                ...
                if(times != 0){
                    for (int i = 0; i < times; i++) {
                        ...
                        if(!result){
                            [ErroruserInfo setObject:@"0216" forKey:@"ErrorCode"];
                            [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                            return;
                        }else{
...
//                        sleep(0.5);
                ...
                if(last > 0){
                    ...
                    bool result =  [self sendCommonBinaryData:data_packet];
                    if(!result){
                        [ErroruserInfo setObject:@"0217" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }else{
...
//                    sleep(0.5);
               ...
               result =  [self sendCommonData:send_data_end_1];
                if(result == false){
                    [ErroruserInfo setObject:@"0218" forKey:@"ErrorCode"];
                    [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
               result =  [self sendCommonData:send_data_end_2];
                if(result == false){
                    [ErroruserInfo setObject:@"0219" forKey:@"ErrorCode"];
                    [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
                    ...
                    if(resu){
                        if([succsendend isEqualToString:attribute_00]){
                         ...
                         result = [self sendCommonData:check_succ_end_packet_2];
                            if(result == false){
                                [ErroruserInfo setObject:@"0220" forKey:@"ErrorCode"];
                                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                                return;
                            }
                        }
                ...
                if([is1101 isEqualToString:attribute_01]){
                   ...
                   BOOL result = [self sendCommonData:send_1101_data_packet];
                    if(result == false){
                        [ErroruserInfo setObject:@"0221" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }
                ...
                if([isspecdata isEqualToString:attribute_01]){
                    ...
                    if(result == false){
                        [ErroruserInfo setObject:@"0222" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result =  [self sendCommonData:send_spec_data_packet_2];
                    if(result == false){
                        [ErroruserInfo setObject:@"0223" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result =  [self sendCommonData:send_spec_data_packet_3];
                    if(result == false){
                        [ErroruserInfo setObject:@"0224" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result =  [self sendCommonData:send_spec_data_packet_4];
                    if(result == false){
                        [ErroruserInfo setObject:@"0225" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result =  [self sendCommonData:send_spec_data_packet_5];
                    if(result == false){
                        [ErroruserInfo setObject:@"0226" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result =  [self sendCommonData:send_spec_data_packet_6];
                    if(result == false){
                        [ErroruserInfo setObject:@"0227" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result =  [self sendCommonData:send_spec_data_packet_7];
                    if(result == false){
                        [ErroruserInfo setObject:@"0228" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }
        ...
        if([end1 isEqualToString:attribute_01]){
          ...
          BOOL result =  [self sendCommonData:send_end_1_packet_1];
            ...
            if(result == false){
                [ErroruserInfo setObject:@"0229" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =  [self sendCommonData:send_end_1_packet_3];
            if(result == false){
                [ErroruserInfo setObject:@"0230" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_1_packet_4];
            if(result == false){
                [ErroruserInfo setObject:@"0231" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_1_packet_5];
            if(result == false){
                [ErroruserInfo setObject:@"0232" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_1_packet_6];
            if(result == false){
                [ErroruserInfo setObject:@"0233" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_1_packet_7];
            if(result == false){
                [ErroruserInfo setObject:@"0234" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_1_packet_8];
            if(result == false){
                [ErroruserInfo setObject:@"0235" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
        ...
        if([end2 isEqualToString:attribute_01]){
            ...
            if(result == false){
                [ErroruserInfo setObject:@"0236" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            ...
            OSSpinLockLock(&oslock);
            if(result == false){
                [ErroruserInfo setObject:@"0237" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
...
//        NSString * path = [[NSUserDefaults standardUserDefaults] objectForKey:vin_file_path];
        ...
        for (NSString * subString in cafdString) {
            if(subString.length > 0){
               ...
               BOOL result =  [self sendCommonData:dataString];
                if(result == false){
                    [ErroruserInfo setObject:@"0238" forKey:@"ErrorCode"];
                    [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
            }
        ...
        if([end4 isEqualToString:attribute_01]){
            ...
            if(result == false){
                [ErroruserInfo setObject:@"0239" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
...
//            [self sendCommonData:send_end_4_packet_2_unit_1];
            ...
            if(result == false){
                [ErroruserInfo setObject:@"0240" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_4_packet_3];
            if(result == false){
                [ErroruserInfo setObject:@"0241" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_4_packet_4];
            if(result == false){
                [ErroruserInfo setObject:@"0242" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_4_packet_5];
            if(result == false){
                [ErroruserInfo setObject:@"0243" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:send_end_4_packet_6];
            if(result == false){
                [ErroruserInfo setObject:@"0244" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
