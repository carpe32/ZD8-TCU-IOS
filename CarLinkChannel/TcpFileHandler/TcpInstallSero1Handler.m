...
#import "TcpInstallSero1Handler.h"
#import "HttpClient.h"
...
@interface TcpInstallSero1Handler()
{
...
@implementation TcpInstallSero1Handler
       ...
       // }
        ...
        if([front2 isEqualToString:condition_on]){
          ...
          BOOL result = [self sendCommonData:front2_data_packet_1];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0101" forKey:@"ErrorCode"];
            ...
            if(result == FALSE){
                [ErroruserInfo setObject:@"0102" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_3];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0103" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_4];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0104" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_5];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0105" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_6];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0106" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result = [self sendCommonData:front2_data_packet_7];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0107" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
        ...
        for (int i = 0; i < 14; i++) {
            ...
            if([iswrite isEqualToString:attribute_01]){
                if([issafe isEqualToString:condition_on]){
                    ...
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0108" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    ...
                    [interface getanquansuanfabtld:self.btldValue parmarsvar3:receiveanquan doneBlock:^(NSString * suanfa){
                        ...
                        NSString * packetString = [safe_data_packet_3_unit_1 stringByAppendingString:suanfa];
                    ...
                    OSSpinLockLock(&oslock);
                    if(result == false){
                        [ErroruserInfo setObject:@"0109" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    ...
                    result =  [self sendCommonData:safe_data_packet_4];
                    if(result == false){
                        [ErroruserInfo setObject:@"0110" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    //  [];
                ...
                if([issend isEqualToString:attribute_01]){
                    ...
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0111" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    ...
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0112" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    ...
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0113" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    ...
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0114" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                ...
                if(result == FALSE){
                    [ErroruserInfo setObject:@"0115" forKey:@"ErrorCode"];
                    [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
             ...
             //   NSString * binString = [NSData hexStringFromHexData:binData];
                ...
                if(times != 0){
                    for (int i = 0; i < times; i++) {
                        ...
                        if(!result){
                            [ErroruserInfo setObject:@"0116" forKey:@"ErrorCode"];
                            [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                ...
                if(last > 0){
                    ...
                    bool result =  [self sendCommonBinaryData:data_packet];
                    if(!result){
                        [ErroruserInfo setObject:@"0117" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                ...
                if(result == FALSE){
                    [ErroruserInfo setObject:@"0118" forKey:@"ErrorCode"];
                    [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                ...
                if(result == FALSE){
                    [ErroruserInfo setObject:@"0119" forKey:@"ErrorCode"];
                    [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                ...
                if([ischecksuccess isEqualToString:attribute_01]){
                    ...
                    if(resu){
                        if([succsendend isEqualToString:attribute_00]){
                            // 如果发送成功
                          resu = [self sendCommonData:check_succ_end_packet_2];
                            if(result == FALSE){
                                [ErroruserInfo setObject:@"0120" forKey:@"ErrorCode"];
                                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                ...
                if([is1101 isEqualToString:attribute_01]){
                   ...
                   result = [self sendCommonData:send_1101_data_packet];
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0121" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }
                ...
                if([isspecdata isEqualToString:attribute_01]){
                    ...
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0122" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_2];
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0123" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_3];
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0124" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_4];
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0125" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_5];
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0126" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_6];
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0127" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_7];
                    if(result == FALSE){
                        [ErroruserInfo setObject:@"0128" forKey:@"ErrorCode"];
                        [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }
        ...
        if([end1 isEqualToString:attribute_01]){
          ...
          BOOL result = [self sendCommonData:send_end_1_packet_1];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0129" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           ...
           result = [self sendCommonData:datapcket];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0130" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_3];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0131" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_4];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0132" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_5];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0133" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_6];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0134" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_7];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0135" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_8];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0136" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
        ...
        if([end2 isEqualToString:attribute_01]){
            ...
            if(result == FALSE){
                [ErroruserInfo setObject:@"0137" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            ...
            [interface getanquansuanfabtld:self.btldValue parmarsvar3:receiveanquan doneBlock:^(NSString * anquan){
                NSString * packetString = [send_end_2_packet_3_unit_1 stringByAppendingString:anquan];
                result = [self sendCommonData:packetString];
                OSSpinLockUnlock(&oslock);
            } withError:^(NSError * error){
                result = false;
...
//        NSString * path = [[NSUserDefaults standardUserDefaults] objectForKey:vin_file_path];
        ...
        for (NSString * subString in cafdString) {
            if(subString.length > 0){
                ...
                BOOL result = [self sendCommonData:dataString];
                if(result == FALSE){
                    [ErroruserInfo setObject:@"0138" forKey:@"ErrorCode"];
                    [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
            }
        ...
        if([end4 isEqualToString:attribute_01]){
            ...
            if(result == FALSE){
                [ErroruserInfo setObject:@"0139" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
...
//            [self sendCommonData:send_end_4_packet_2_unit_1];
           result =  [self sendCommonData:dataPacket];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0140" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_4_packet_3];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0141" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_4_packet_4];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0142" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_4_packet_5];
            if(result == FALSE){
                [ErroruserInfo setObject:@"0143" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
