//
//  TcpInstallSero5Handler.m
//  CarLinkChannel
//
//  Created by job on 2023/4/21.
//

#import "TcpInstallSero5Handler.h"
#import "HttpClient.h"
...
@interface TcpInstallSero5Handler()
{
    __block OSSpinLock oslock;
    NSString * receiveanquan;
}
@end
@implementation TcpInstallSero5Handler
       ...
       // }
        ...
        if([front2 isEqualToString:condition_on]){
           ...
           BOOL result = [self sendCommonData:front2_data_packet_1];
            if(result == false){
                [ErroruserInfo setObject:@"0501" forKey:@"ErrorCode"];
            ...
            if(result == false){
                [ErroruserInfo setObject:@"0502" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =  [self sendCommonData:front2_data_packet_3];
            if(result == false){
                [ErroruserInfo setObject:@"0503" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =  [self sendCommonData:front2_data_packet_4];
            if(result == false){
                [ErroruserInfo setObject:@"0504" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =  [self sendCommonData:front2_data_packet_5];
            if(result == false){
                [ErroruserInfo setObject:@"0505" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =  [self sendCommonData:front2_data_packet_6];
            if(result == false){
                [ErroruserInfo setObject:@"0506" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            result =  [self sendCommonData:front2_data_packet_7];
            if(result == false){
                [ErroruserInfo setObject:@"0507" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
        ...
        for (int i = 0; i < 14; i++) {
            ...
            if([iswrite isEqualToString:attribute_01]){
                if([issafe isEqualToString:condition_on]){
                    ...
                    if(result == false){
                        [ErroruserInfo setObject:@"0508" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
...
//                    NSString * charstring = [var3 stringByAddingPercentEncodingWithAllowedCharacters:charset];
                  ...
                  //  NSString * strurl = [NSString stringWithFormat:@"%@%@",anquan_suanfa_url,receiveanquan];
                    ...
                    [interface getanquansuanfabtld:self.btldValue parmarsvar3:receiveanquan doneBlock:^(NSString * suanfa){
                        ...
                        NSString * packetString = [sero_2_datapcket_1_anquan_prefix stringByAppendingString:suanfa];
...
//
                    OSSpinLockLock(&oslock);
                    if(result == false){
                        [ErroruserInfo setObject:@"0509" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  ...
                  result =  [self sendCommonData:sero_2_datapcket_1];
                    if(result == false){
                        [ErroruserInfo setObject:@"0510" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                  //  [];
                ...
                if([issend isEqualToString:attribute_01]){
                    ...
                    if(result == false){
                        [ErroruserInfo setObject:@"0511" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
...
//
                ...
                if(result == false){
                    [ErroruserInfo setObject:@"0512" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
            ...
            //    NSLog(@"binString: %@",binString);
                ...
                if(times != 0){
                    for (int i = 0; i < times; i++) {
                        ...
                        if(!result){
                            [ErroruserInfo setObject:@"0513" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                            return;
                        }else{
...
//                        sleep(0.5);
                ...
                if(last > 0){
                    ...
                    bool result =  [self sendCommonBinaryData:data_packet];
                    if(!result){
                        [ErroruserInfo setObject:@"0514" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }else{
...
//                    sleep(0.5);
               ...
               result = [self sendCommonData:send_data_end_1];
                if(result == false){
                    [ErroruserInfo setObject:@"0515" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
               result = [self sendCommonData:send_data_end_2];
                if(result == false){
                    [ErroruserInfo setObject:@"0516" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
                ...
                if([ischecksuccess isEqualToString:attribute_01]){
                    ...
                    if(resu){
                        if([succsendend isEqualToString:attribute_00]){
                            // 如果发送成功
                            bool result = [self sendCommonData:check_succ_end_packet_2];
                            if(result == false){
                                [ErroruserInfo setObject:@"0517" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                                return;
                            }
                        }
                ...
                if([is1101 isEqualToString:attribute_01]){
                   ...
                   BOOL result =  [self sendCommonData:send_1101_data_packet];
                    if(result == false){
                        [ErroruserInfo setObject:@"0518" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }
                ...
                if([isspecdata isEqualToString:attribute_01]){
                   ...
                   BOOL result = [self sendCommonData:send_spec_data_packet_1];
                    if(result == false){
                        [ErroruserInfo setObject:@"0519" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_2];
                    if(result == false){
                        [ErroruserInfo setObject:@"0520" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_3];
                    if(result == false){
                        [ErroruserInfo setObject:@"0521" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_4];
                    if(result == false){
                        [ErroruserInfo setObject:@"0522" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_5];
                    if(result == false){
                        [ErroruserInfo setObject:@"0523" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_6];
                    if(result == false){
                        [ErroruserInfo setObject:@"0524" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                    result = [self sendCommonData:send_spec_data_packet_7];
                    if(result == false){
                        [ErroruserInfo setObject:@"0525" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                        return;
                    }
                }
        ...
        if([end1 isEqualToString:attribute_01]){
           ...
           BOOL result = [self sendCommonData:send_end_1_packet_1];
            if(result == false){
                [ErroruserInfo setObject:@"0526" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           ...
           result = [self sendCommonData:datapcket];
            if(result == false){
                [ErroruserInfo setObject:@"0527" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_3];
            if(result == false){
                [ErroruserInfo setObject:@"0528" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_4];
            if(result == false){
                [ErroruserInfo setObject:@"0529" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_5];
            if(result == false){
                [ErroruserInfo setObject:@"0530" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_6];
            if(result == false){
                [ErroruserInfo setObject:@"0531" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_7];
            if(result == false){
                [ErroruserInfo setObject:@"0532" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_1_packet_8];
            if(result == false){
                [ErroruserInfo setObject:@"0533" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
        ...
        if([end2 isEqualToString:attribute_01]){
            ...
            if(result == false){
                [ErroruserInfo setObject:@"0534" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
            ...
            OSSpinLockLock(&oslock);
            if(result == false){
                [ErroruserInfo setObject:@"0535" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
        }
...
//        NSString * path = [[NSUserDefaults standardUserDefaults] objectForKey:vin_file_path];
        ...
        for (NSString * subString in cafdString) {
            if(subString.length > 0){
                ...
                BOOL result =  [self sendCommonData:dataString];
                if(result == false){
                    [ErroruserInfo setObject:@"0536" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                    return;
                }
            }
        ...
        if([end4 isEqualToString:attribute_01]){
           ...
           BOOL result = [self sendCommonData:send_end_4_packet_1];
            if(result == false){
                [ErroruserInfo setObject:@"0537" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
...
//            [self sendCommonData:send_end_4_packet_2_unit_1];
           result =  [self sendCommonData:dataPacket];
            if(result == false){
                [ErroruserInfo setObject:@"0538" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result =  [self sendCommonData:send_end_4_packet_3];
            if(result == false){
                [ErroruserInfo setObject:@"0539" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_4_packet_4];
            if(result == false){
                [ErroruserInfo setObject:@"0540" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
           result = [self sendCommonData:send_end_4_packet_5];
            if(result == false){
                [ErroruserInfo setObject:@"0541" forKey:@"ErrorCode"];
                [[NSNotificationCenter defaultCenter] postNotificationName:fail_install_notify_name object:nil userInfo:ErroruserInfo];
                return;
            }
          //  [self sendCommonData:send_end_4_packet_6];
        }
